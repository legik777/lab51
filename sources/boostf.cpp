...
std::vector<std::string> recdir(std::string path_to_ftp) {
    ...
    int countv=0;
    for (boost::filesystem::recursive_directory_iterator it(path_to_ftp); it != end; ++it) { //перебор файлов и каталогов
        boost::filesystem::file_status fs = boost::filesystem::status(*it); //информация о файле
        switch (fs.type()) { //
        ...
        if (!boost::algorithm::ends_with(fi, ".old") && boost::algorithm::starts_with(fi, "balance_")) { //
            countv++;
